#include "TJNJConstraintSolver.h"
#include "SharedFuncsForRigidBody.h"


namespace dyno
{
	IMPLEMENT_TCLASS(TJNJConstraintSolver, TDataType)

	template<typename TDataType>
	TJNJConstraintSolver<TDataType>::TJNJConstraintSolver()
		:ConstraintModule()
	{
	...
	template<typename TDataType>
	void TJNJConstraintSolver<TDataType>::initializeJacobian(Real dt)
	{
	...
	template<typename TDataType>
	void TJNJConstraintSolver<TDataType>::initializeJacobianForNJ()
	{
		int constraint_size = 0;
		int contact_size = this->inContacts()->size();
		...
		if (mPositionConstraints.size() != constraint_size)
		{
	...
	template<typename TDataType>
	void TJNJConstraintSolver<TDataType>::constrain()
	{
		...
		if (!this->inContacts()->isEmpty() || topo->totalJointSize() > 0)
		{
			...
			for (int i = 0; i < this->varSubStepping()->getValue(); i++)
			{
				...
				//Position Solver
				for (int j = 0; j < this->varIterationNumberForPositionSolver()->getValue(); j++)
				{
					mImpulseC.reset();
					mLambda.reset();
					initializeJacobianForNJ();
					for (int k = 0; k < 2; k++)
					{
	...
	DEFINE_CLASS(TJNJConstraintSolver);
}