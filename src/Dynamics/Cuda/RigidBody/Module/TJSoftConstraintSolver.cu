...
namespace dyno
{
	...
	template<typename TDataType>
	void TJSoftConstraintSolver<TDataType>::damagedJoints(Real dt)
	{
	...
	template<typename TDataType>
	void TJSoftConstraintSolver<TDataType>::initializeRelaxation()
	{
			...
			for (int i = 0; i < this->varSubStepping()->getValue(); i++)
			{
				...
				updateGesture(
					...
					dh
						...
						for (int k = 0; k < this->varIterationNumberForVelocitySolver()->getValue(); k++)
						{
							JacobiIteration(
								mLambda,
								mImpulseC,
