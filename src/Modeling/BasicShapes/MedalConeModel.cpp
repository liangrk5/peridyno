...
namespace dyno
{
	...
	MedalConeModel<TDataType>::MedalConeModel()
		: BasicShape<TDataType>()
	{
		...
		auto callback = std::make_shared<FCallBackFunc>(std::bind(&MedalConeModel<TDataType>::varChanged, this));
	...
	template<typename TDataType>
	void MedalConeModel<TDataType>::resetStates()
	{
		varChanged();
	}

	template<typename TDataType>
	NBoundingBox MedalConeModel<TDataType>::boundingBox()
	{
	...
	template<typename TDataType>
	void MedalConeModel<TDataType>::varChanged()
	{
		...
		TMedalCone3D<Real> medalConePrim = TMedalCone3D<Real>(center, q, pA, pB, rA, rB);

		this->outMedalCone()->setValue(medalConePrim);

		std::vector<Vec3f> vertices;
		std::vector<TopologyModule::Triangle> triangles;

		pushback_medalslab(vertices, triangles, pA_world, pB_world, rA, rB);
	...
	template<typename TDataType>
	double MedalConeModel<TDataType>::compute_angle(double r1, double r2, const Vec3f& c21) {
	...
	template<typename TDataType>
	Vec3f MedalConeModel<TDataType>::plane_line_intersection(const Vec3f& n, const Vec3f& p,
	...
	template<typename TDataType>
	Mat4f MedalConeModel<TDataType>::rotate_mat(const Vec3f& point, const Vec3f& axis, double angle) {
	...
	template<typename TDataType>
	Vec3f MedalConeModel<TDataType>::mat_vec_mult(const Mat4f& mat,
		const Vec3f& vec)
	{
	...
	template<typename TDataType>
	std::vector<Vec3f> MedalConeModel<TDataType>::intersect_point_of_cones(const Vec3f& v1, double r1,
	...
	template<typename TDataType>
	int MedalConeModel<TDataType>::generate_slab(
		const Vec3f& v1,
		double r1,
		const Vec3f& v2,
		double r2,
	...
	template<typename TDataType>
	void MedalConeModel<TDataType>::generate_conical_surface(
		const Vec3f& v1,
		float r1,
		const Vec3f& v2,
		float r2,
	...
	template<typename TDataType>
	void MedalConeModel<TDataType>::pushback_medalslab(std::vector<Vec3f>& vertices, std::vector<TopologyModule::Triangle>& triangles, Vec3f pA, Vec3f pB, Real rA, Real rB, uint resolution)
	{
