...
namespace dyno
{
	...
	MedalSlabModel<TDataType>::MedalSlabModel()
		: BasicShape<TDataType>()
	{
		...
		auto callback = std::make_shared<FCallBackFunc>(std::bind(&MedalSlabModel<TDataType>::varChanged, this));
		...
		this->varRadiusC()->attach(callback);
	...
	template<typename TDataType>
	void MedalSlabModel<TDataType>::resetStates()
	{
		varChanged();
	}

	template<typename TDataType>
	NBoundingBox MedalSlabModel<TDataType>::boundingBox()
	{
	...
	template<typename TDataType>
	void MedalSlabModel<TDataType>::varChanged()
	{
		...
		Vec3f pA = this->varPointA()->getValue();
		Vec3f pB = this->varPointB()->getValue();
		Vec3f pC = this->varPointC()->getValue();

		Real rA = this->varRadiusA()->getValue();
		Real rB = this->varRadiusB()->getValue();
		Real rC = this->varRadiusC()->getValue();

		Vec3f pA_world = center + q.rotate(pA * scale);
		Vec3f pB_world = center + q.rotate(pB * scale);
		Vec3f pC_world = center + q.rotate(pC * scale);
		...
		pushback_medalslab(vertices, triangles, pA_world, pB_world, pC_world, rA, rB, rC);
	...
	template<typename TDataType>
	double MedalSlabModel<TDataType>::compute_angle(double r1, double r2, const Vec3f& c21) {
	...
	template<typename TDataType>
	Vec3f MedalSlabModel<TDataType>::plane_line_intersection(const Vec3f& n, const Vec3f& p,
	...
	template<typename TDataType>
	Mat4f MedalSlabModel<TDataType>::rotate_mat(const Vec3f& point, const Vec3f& axis, double angle) {
	...
	template<typename TDataType>
	Vec3f MedalSlabModel<TDataType>::mat_vec_mult(const Mat4f& mat,
		const Vec3f& vec)
	{
	...
	template<typename TDataType>
	std::vector<Vec3f> MedalSlabModel<TDataType>::intersect_point_of_cones(const Vec3f& v1, double r1,
	...
	template<typename TDataType>
	int MedalSlabModel<TDataType>::generate_slab(
		const Vec3f& v1,
		double r1,
		const Vec3f& v2,
		double r2,
	...
	template<typename TDataType>
	void MedalSlabModel<TDataType>::generate_conical_surface(
		const Vec3f& v1,
		float r1,
		const Vec3f& v2,
		float r2,
	...
	template<typename TDataType>
	void MedalSlabModel<TDataType>::pushback_medalslab(std::vector<Vec3f>& vertices, std::vector<TopologyModule::Triangle>& triangles, Vec3f pA, Vec3f pB, Vec3f pC, Real rA, Real rB, Real rC, uint resolution)
	{

		generate_conical_surface(pA, rA, pB, rB, resolution, vertices, triangles);
		generate_conical_surface(pB, rB, pC, rC, resolution, vertices, triangles);
		generate_conical_surface(pC, rC, pA, rA, resolution, vertices, triangles);

		generate_slab(pA, rA, pB, rB, pC, rC, vertices, triangles);


		for (size_t i = 0; i < 3; i++)
		{
			Real r = i == 0 ? rA : (i == 1 ? rB : rC);
			Vec3f p = i == 0 ? pA : (i == 1 ? pB : pC);
