...
namespace dyno
{
	...
	class MedalConeModel : public BasicShape<TDataType>
	{
		DECLARE_TCLASS(MedalConeModel, TDataType);
	...
	public:
		...
		std::string caption() override { return "Medal Cone"; }

		BasicShapeType getShapeType() override { return BasicShapeType::MEDALCONE; }
	...
	public:
		...
		DEF_VAR_OUT(TMedalCone3D<Real>, MedalCone, "");

	public:

		static void pushback_medalslab(std::vector<Vec3f>& vertices, std::vector<TopologyModule::Triangle>& triangles, Vec3f pA, Vec3f pB, Real rA, Real rB, uint resolution = 40);
	...
	IMPLEMENT_TCLASS(MedalConeModel, TDataType);
}