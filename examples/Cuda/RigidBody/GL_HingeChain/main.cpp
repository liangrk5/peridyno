...
std::shared_ptr<SceneGraph> creatBricks()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		Real scale = 2.5;
		SphereInfo sphere;
		sphere.center = scale * Vec3f(-2.3, 5, 0.5 + 2.0 * i);
		sphere.radius = scale * 0.2;
		auto sphereActor = rigid->addSphere(sphere, rigidBody, 5 * pow(100,i));
		BoxInfo newbox, oldbox;
		oldbox.center = scale * Vec3f(-2.0, 5, 0.5 + 2.0 * i);
		...
		//hingeJoint1.setMaxForceAndTorque(10000, 7000);
		...
		for (int i = 0; i < 20; i++)
		{
			...
			//hingeJoint.setMaxForceAndTorque(10000, 7000);
			oldbox = newbox;
